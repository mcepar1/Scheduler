...
from global_vars import doctors
...
class DoctorPanel(wx.Panel):
  ...
  def __init__(self, parent):
    ...
    # add event listeners
    self.Bind(wx.grid.EVT_GRID_LABEL_LEFT_CLICK, self.doctor_selected, self.grid)
    
  def fill_grid(self):
    table = doctors.as_table()
  ...
  def doctor_selected(self, event):
    if event.GetCol() == -1:
      if event.GetRow() < 0:
        ...
        self.person.set_person(doctors.get_element(event.GetRow()))
